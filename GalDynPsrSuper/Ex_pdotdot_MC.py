...
from . import ExGal_pdot_MC
from . import ExShk_pdot



def pdotdotexMC(ldeg,bdeg,dkpc,mul,mub,p,pdotobs,vrad):
    ...
    pex_pl = ExGal_pdot_MC.Expl(ldeg, bdeg, dkpc)

    pex_z = ExGal_pdot_MC.Exz(ldeg, bdeg, dkpc)

    pex_shk = ExShk_pdot.Exshk(dkpc, mul, mub)
    
    pex_tot = pex_pl + pex_z + pex_shk
    ...
    res2 = -2.*pex_tot*(pex_tot + math.cos(b)*math.cos(l)*(aspl/c) + (mastorad/yrts)*mub*(1000.*Vs/c)*math.sin(b)*math.sin(l) - (mastorad/yrts)*mul*(1000.*Vs/c)*math.cos(l))
    ...
    fourthterm = 2.*pex_tot*(pdotobs/p)
